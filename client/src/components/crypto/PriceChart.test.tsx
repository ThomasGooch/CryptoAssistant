...
const mockCryptoService = cryptoService as unknown as {
  getHistoricalPrices: ReturnType<typeof vi.fn>;
};
...
describe("PriceChart", () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    vi.mocked(mockCryptoService.getHistoricalPrices).mockResolvedValue({
      symbol: "BTC",
  ...
  test("fetches and displays historical price data", async () => {
    const { container } = render(
      <PriceChart symbol="BTC" timeframe={Timeframe.Hour} />,
    );
  ...
  test("refreshes data periodically", { timeout: 10000 }, async () => {
    ...
    expect(mockCryptoService.getHistoricalPrices).toHaveBeenLastCalledWith(
      "BTC",
      Timeframe.Hour,
    );
