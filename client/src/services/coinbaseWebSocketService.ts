...
class CoinbaseWebSocketService {
    ...
    try {
      const url =
        this.currentUrlIndex >= 0
          ? this.fallbackUrls[this.currentUrlIndex]
          : this.wsUrl;
      ...
      this.ws.onclose = (event) => {
        ...
        console.log(
          `Consecutive failures: ${this.consecutiveFailures}/${this.maxConsecutiveFailures}`,
        );
        ...
        if (this.consecutiveFailures >= this.maxConsecutiveFailures) {
          console.warn(
            `⚠️ Too many consecutive WebSocket failures (${this.consecutiveFailures}). Enabling mock mode.`,
          );
          this.enableMockMode();
          return;
        }
  ...
  private handleConnectionFailure(): void {
    this.consecutiveFailures++;
    console.log(
      `Connection failure. Consecutive failures: ${this.consecutiveFailures}/${this.maxConsecutiveFailures}`,
    );
    ...
    if (this.currentUrlIndex < this.fallbackUrls.length - 1) {
      ...
      console.log(
        `🔄 Trying fallback URL: ${this.fallbackUrls[this.currentUrlIndex]}`,
      );
      ...
      console.error(
        "❌ All WebSocket connection attempts failed - enabling mock mode",
      );
      this.enableMockMode();
    }
  }
    ...
    switch (code) {
      ...
      case 1003:
        return {
          ...
          reason: "Unsupported data type received",
        };
      case 1005:
      ...
      case 1006:
        return {
          ...
          reason: "Connection lost unexpectedly (network/firewall issue)",
        };
      case 1007:
        return {
          meaning: "Invalid Frame Data",
          reason: "Invalid UTF-8 data received",
        };
      case 1008:
        return {
      ...
      case 1010:
        return {
          ...
          reason: "Required extension not supported",
        };
      case 1011:
        return {
  ...
  private enableMockMode(): void {
    this.mockMode = true;
    console.log(
      "🤖 WebSocket mock mode enabled - generating simulated price data",
    );
    console.log(
      "ℹ️ Mock mode provides realistic price movements for development/testing",
    );
  ...
  private startMockData(): void {
    ...
    this.mockTimer = window.setInterval(() => {
      this.priceCallbacks.forEach((callback, productId) => {
        ...
        const basePrice =
          symbol === "BTC" ? 115000 : symbol === "ETH" ? 3500 : 1000;
        ...
        const mockPrice: CryptoPrice = {
          ...
          percentChange24h: parseFloat(
            ((change24h / basePrice) * 100).toFixed(2),
          ),
        };
