...
class CoinbaseWebSocketService {
   ...
   */
  public subscribeToTicker(symbol: string, callback: (price: CryptoPrice) => void): void {
    if (!symbol.trim()) {
      return;
    }
  ...
  private unsubscribeFromAll(): void {
    ...
    if (productIds.length > 0 && this.ws && this.ws.readyState === WebSocket.OPEN) {
   ...
   */
  public offConnectionStateChange(callback: (connected: boolean) => void): void {
  ...
  private scheduleReconnect(): void {
    this.reconnectCount++;
    console.log(`Attempting to reconnect (${this.reconnectCount}/${this.reconnectAttempts}) in ${this.reconnectDelay}ms...`);
    ...
    if (callback) {
      try {
        const price: CryptoPrice = {
          ...
          priceChange24h: parseFloat(ticker.price) - parseFloat(ticker.open_24h),
          percentChange24h: ((parseFloat(ticker.price) - parseFloat(ticker.open_24h)) / parseFloat(ticker.open_24h)) * 100
        };
  ...
  private sendSubscription(type: "subscribe" | "unsubscribe", productIds: string[]): void {
    if (!this.ws || this.ws.readyState !== WebSocket.OPEN) {
    ...
    console.log(`${type === "subscribe" ? "Subscribing to" : "Unsubscribing from"} products:`, productIds);
